...
static void simpleBLECentral_HandleKeys( uint8 shift, uint8 keys )
{
  ...
  if ( keys & HAL_KEY_LEFT )
  {
    ...
    if ( !simpleBLEScanning && simpleBLEScanRes > 0 )
    {
        ...
        printf( "Device %d \n", simpleBLEScanIdx + 1);
        printf( bdAddr2Str( simpleBLEDevList[simpleBLEScanIdx].addr ));
        printf("\n");
        simpleBLECentral_HandleKeys(NULL, HAL_KEY_CENTER);
    }
  }
...
static uint8 simpleBLECentralEventCB( gapCentralRoleEvent_t *pEvent )
{
  switch ( pEvent->gap.opcode )
  {
    case GAP_DEVICE_INIT_DONE_EVENT:  
      {
        printf( "BLE Central \n" );
        printf( bdAddr2Str( pEvent->initDone.devAddr ) );
        printf("\n");
        
  //JK_TEST
simpleBLECentral_HandleKeys(NULL, HAL_KEY_UP)   ;
      }
      break;
    ...
    case GAP_DEVICE_DISCOVERY_EVENT:
      {
        ...
        if ( simpleBLEScanRes > 0 )
        {
          printf( "<- To Select\n" );
          //JK_TEST
        simpleBLECentral_HandleKeys(NULL, HAL_KEY_LEFT)   ;
        }
        else 
        {
          //JK_TEST
    ...
    case GAP_LINK_ESTABLISHED_EVENT:
      {
        ...
        {          
          ...
          printf( "c\n " );
          //printf( bdAddr2Str( pEvent->linkCmpl.devAddr ) );   
          //printf("\n");
        }
        else
        {
                                        ...
                                        uint8 uiInputs, uint8 uiOutputs )
{
  ...
  {
    printf( "Passcode:" );
    printf( (char *) _ltoa(passcode, str, 10) );
    printf("\n");
  }
...
static void simpleBLEGATTDiscoveryEvent( gattMsgEvent_t *pMsg )
{
  ...
  printf("mm");
        ...
        req.type.uuid[0] = LO_UINT16(0);//SIMPLEPROFILE_CHAR1_UUID
        req.type.uuid[1] = HI_UINT16(0);//SIMPLEPROFILE_CHAR1_UUID
